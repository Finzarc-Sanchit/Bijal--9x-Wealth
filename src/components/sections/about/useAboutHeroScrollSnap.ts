...
import {
  ABOUT_HERO_STEP_COUNT,
  aboutStepToScrollProgress,
  aboutScrollProgressToStep,
...
export function useAboutHeroScrollSnap(containerRef: RefObject<HTMLDivElement | null>) {
  ...
  const scrollToStep = useCallback(
    (step: number, behavior: ScrollBehavior = "smooth") => {
      ...
      const clamped = Math.min(ABOUT_HERO_STEP_COUNT - 1, Math.max(0, step));
      const range = el.offsetHeight - window.innerHeight;
      const top = el.offsetTop + range * aboutStepToScrollProgress(clamped);
  ...
  const getCurrentStep = useCallback(() => {
    const el = containerRef.current;
    if (!el) return 0;
    return aboutScrollProgressToStep(getAboutHeroScrollProgress(el));
  }, [containerRef]);
  ...
  useEffect(() => {
    const onWheel = (e: WheelEvent) => {
      ...
      if (e.deltaY > WHEEL_THRESHOLD) {
        if (step < ABOUT_HERO_STEP_COUNT - 1) {
          e.preventDefault();
          scrollToStep(step + 1);
        }
  ...
  useEffect(() => {
    ...
    const onTouchEnd = (e: TouchEvent) => {
      ...
      const step = getCurrentStep();
      if (delta > 0 && step < ABOUT_HERO_STEP_COUNT - 1) {
        scrollToStep(step + 1);
  ...
  return { scrollToStep, getCurrentStep };
}
