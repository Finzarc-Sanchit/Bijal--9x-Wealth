...
export type PracticeAreaGridItem = {
  id: string;
  title: string;
  description: string;
  cta: string;
...
export type PracticeAreasGridProps = {
  eyebrow: string;
  heading: readonly [string, string];
  description: string;
  items: readonly PracticeAreaGridItem[];
  className?: string;
  stackPinned?: boolean;
  id?: string;
};
...
function PracticeAreaCard({
  ...
  return (
    <div
      className={cn(
        "relative flex h-full min-h-[360px] flex-col overflow-hidden rounded-xl p-8 transition-all duration-500 ease-out sm:min-h-[400px] sm:p-10 lg:min-h-[440px] lg:p-12",
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-10 flex h-full w-full grow flex-col items-start justify-between"
      >
        <motion.div
          layout="position"
          transition={INSIDE_SPRING}
          className={cn("flex w-full flex-col", isActive ? "" : "mt-auto")}
        >
          <motion.h3
            layout="position"
            transition={INSIDE_SPRING}
            className={cn(
              ...
              isActive
                ...
                : "text-2xl font-normal md:text-3xl",
            )}
          >
            {item.title}
          </motion.h3>
        ...
        <Link
          href={item.href}
          className={cn(
            "group relative mt-10 inline-flex min-h-[44px] w-fit cursor-pointer items-center gap-4 self-start transition-all duration-500",
            isActive
          ...
          <div className="relative min-w-0 w-full overflow-hidden">
            ...
            >
              {items.map((item, index) => (
                <div
                  key={item.id}
                  className="flex min-w-0 w-[72%] shrink-0 flex-col gap-6 pr-8 lg:flex-row lg:items-stretch"
                >
                  <PracticeAreaCard item={item} isActive={activeIndex === index} />
                </div>
              ))}
            </motion.div>
