...
export type ProtectionShowcaseItem = {
  id: string;
  title: string;
  body: string;
  src: string;
...
export type ProtectionShowcaseProps = {
  eyebrow: string;
  heading: string;
  description: string;
  items: readonly ProtectionShowcaseItem[];
  className?: string;
  stackPinned?: boolean;
  id?: string;
};
...
export function ProtectionShowcase({
  ...
  className,
  stackPinned = false,
  id = "protection-showcase",
}: ProtectionShowcaseProps) {
    ...
    >
      <div className="mx-auto w-full max-w-full">
        ...
        <div className="mt-14 border-t-2 border-brand-navy md:mt-16">
          {items.map((item) => (
            <ProtectionRow key={item.id} item={item} reduceMotion={reduceMotion} />
          ))}
        </div>
      </div>
    </section>
